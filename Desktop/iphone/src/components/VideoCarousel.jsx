...
const VideoCarousel = () => {
  ...
  const hanleLoadedMetadata = (i, e) => setLoadedData((pre) => [...pre, e])
  ...
  const handleProcess = (type, i) => {
    switch(type) {
      case 'video-end':
        setViedo((prevVideo) => ({...prevVideo, isEnd: true, videoId: i + 1}))
        break;
      
      case 'video-last':
        setViedo((pre) => ({...pre, isLastVideo: true}))
        break;

      case 'video-reset':
        setViedo((pre) => ({...pre, isLastVideo: false, videoId: 0}))
        break;
      
      case 'play':
        setViedo((pre) => ({...pre, isPlaying: !pre.isPlaying }))
        break;
          ...
          <div key={i} id='slider' className='sm:pr-20 pr-10'>
            <div className='video-carousel_container'>
              <div className='w-full h-full flex-centre rounded-3xl overflow-hidden bg-black'>
                ...
                className={`${
                  list.is === 2 && 'translate-x-44'} pointer-events-none`}
                  ...
                  onLoadedMetadata={(e) => hanleLoadedMetadata(i, e)}
                >
